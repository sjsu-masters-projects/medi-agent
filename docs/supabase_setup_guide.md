...
This currently applies migrations `001` through `037`, including canonical clinical facts (`017`), SMART/FHIR import envelopes (`018`), clinical-action approval controls (`019`), database-security hardening (`020`), the least-privilege server-only grants used by the synthetic fixture, A2A retry worker, patient feed, reminder schedules, adherence statistics, SMART import review, clinician review reads (`021`–`028`), safe external-record reconciliation (`029`), guarded document ingestion (`034`), model-invocation telemetry (`035`), the durable document-ingestion worker boundary (`036`), and private derived TIFF previews (`037`). The history contains two `011` filenames; the migration ledger uses full filenames and checksums, making the ordering unambiguous.
