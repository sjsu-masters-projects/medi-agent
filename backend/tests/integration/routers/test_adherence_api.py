...
class TestLogAdherence:
    ...
    def test_success_medication_skipped(
        self, client, override_auth, override_db, mock_supabase_db, patient_id
    ):
    ...
    def test_success_obligation_completed(
        self, client, override_auth, override_db, mock_supabase_db, patient_id
    ):
...
class TestGetAdherenceStats:
    ...
    def test_success_with_data(
        ...
        def table_side_effect(table_name):
            ...
            if table_name == "adherence_logs":
                ...
                table_mock.select().eq().eq().execute.return_value = MagicMock(
    ...
    def test_success_no_data(
        self, client, override_auth, override_db, mock_supabase_db, patient_id
    ):
        """Successfully retrieve stats with no adherence data."""
        # Mock empty logs
        mock_supabase_db.table().select().eq().gte().execute.return_value = MagicMock(data=[])
...
class TestAuthorization:
    ...
    def test_invalid_token(self, client):
        ...
        response = client.get(
            "/api/v1/adherence/stats", headers={"Authorization": "Bearer invalid-token"}
        )
