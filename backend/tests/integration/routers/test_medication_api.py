...
class TestListMedications:
    ...
    def test_success_all_medications(
        self, client, override_auth, override_db, mock_supabase_db, patient_id
    ):
...
class TestCreateMedication:
    ...
    def test_success(self, client, override_auth, override_db, mock_supabase_db, patient_id):
        ...
        mock_supabase_db.table().insert().execute.return_value = MagicMock(data=[medication_data])
    ...
    def test_minimal_fields(self, client, override_auth, override_db, mock_supabase_db, patient_id):
        ...
        response = client.post(
            "/api/v1/medications/",
            json={"name": "Vitamin D", "dosage": "1000 IU", "frequency": "once daily"},
        )
...
class TestUpdateMedication:
    ...
    def test_success(self, client, override_auth, override_db, mock_supabase_db, patient_id):
        ...
        response = client.put(
            ...
            json={"dosage": "20mg", "instructions": "Take in the morning with water"},
        )
    ...
    def test_deactivate_medication(
        self, client, override_auth, override_db, mock_supabase_db, patient_id
    ):
        ...
        response = client.put(
            f"/api/v1/medications/{medication_id}",
            json={"is_active": False, "end_date": "2025-01-15"},
        )
    ...
    def test_not_found(self, client, override_auth, override_db, mock_supabase_db):
        ...
        mock_supabase_db.table().update().eq().eq().execute.return_value = MagicMock(data=[])

        response = client.put(f"/api/v1/medications/{medication_id}", json={"dosage": "20mg"})
...
class TestAuthorization:
    ...
    def test_invalid_token(self, client):
        ...
        response = client.get(
            "/api/v1/medications/", headers={"Authorization": "Bearer invalid-token"}
        )
