...
async def get_rxcui(drug_name: str) -> dict[str, Any]:
    ...
    try:
        async with httpx.AsyncClient() as client:
            ...
            logger.info("rxnorm_rxcui_success", 
                       drug_name=drug_name, 
                       rxcui=rxcui_list[0])
        ...
        logger.error("rxnorm_http_error", 
                    drug_name=drug_name, 
        ...
        logger.error("rxnorm_unexpected_error", 
                    drug_name=drug_name, 
                    error=str(e))
        return {
            "drug_name": drug_name,
...
async def get_drug_properties(rxcui: str) -> dict[str, Any]:
    ...
    try:
        ...
        logger.error("rxnorm_props_http_error", 
                    rxcui=rxcui, 
                    status=e.response.status_code)
        return {"rxcui": rxcui, "error": f"API error: {e.response.status_code}"}
    except Exception as e:
        logger.error("rxnorm_props_unexpected_error", 
                    rxcui=rxcui, 
                    error=str(e))
...
async def get_related_drugs(rxcui: str) -> dict[str, Any]:
    ...
    try:
        ...
        logger.error("rxnorm_related_http_error", 
                    rxcui=rxcui, 
                    status=e.response.status_code)
        return {
            "rxcui": rxcui,
        ...
        logger.error("rxnorm_related_unexpected_error", 
                    rxcui=rxcui, 
                    error=str(e))
        return {
            "rxcui": rxcui,
...
async def normalize_drug_name(drug_name: str) -> dict[str, Any]:
    ...
    try:
        ...
        logger.info("rxnorm_normalize_success", 
                   original=drug_name, 
        ...
        logger.error("rxnorm_normalize_unexpected_error", 
                    drug_name=drug_name, 
                    error=str(e))
        return {
            "original_name": drug_name,
